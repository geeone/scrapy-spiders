...
class CustomRetryMiddleware(RetryMiddleware):
    EXCEPTIONS_TO_RETRY = RetryMiddleware.EXCEPTIONS_TO_RETRY + (ResponseNeverReceived,)


class CustomRetryUserAgentMiddleware(RetryUserAgentMiddleware):
    EXCEPTIONS_TO_RETRY = RetryMiddleware.EXCEPTIONS_TO_RETRY + (ResponseNeverReceived,)
...
class FakeUserAgentMiddleware(RandomUserAgentMiddleware):
    def __init__(self, crawler):
        ...
        try:
            self.ua = UserAgent(fallback='Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/79.0.3945.130 Safari/537.36')
...
class BrobotBotsSpiderMiddleware(object):
...
class BrobotBotsDownloaderMiddleware(object):
    # Not all methods need to be defined. If a method is not defined,
