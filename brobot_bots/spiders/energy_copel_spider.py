...
class energy_copel_spider(CustomSpider):
    # required scraper name
    name = "energy_copel"

    # initial urls
    start_url = "https://www.copel.com/AgenciaWeb/autenticar/loginCliente.do"

    @classmethod
    def from_crawler(cls, crawler, *args, **kwargs):
        """Rewriting of the spider_idle function to yield result after spider closed."""

        spider = super(energy_copel_spider, cls).from_crawler(
            crawler, *args, **kwargs)
    ...
    def agencia_enter(self, response):
        url = "https://www.copel.com/AgenciaWeb/autenticar/autenticar.do"
    ...
    def senha_validation(self, response):
        ...
        url = "https://www.copel.com/AgenciaWeb/autenticar/validarSenha.do"
        yield FormRequest(url, formdata={'senha': self.senha}, callback=self.conta_page, dont_filter=True)
    ...
    def conta_page(self, response):
        ...
        url = "https://www.copel.com/AgenciaWeb/consultarHistoricoPagto/consultarHistoricoPagto.do"
    ...
    def payment_history(self, response):
        ...
        for row in rows:
            ...
            if self.start_date <= vencimento_datetime <= self.end_date:
                ...
                if not data_de_pagamento:
                    referencia_link = "https://www.copel.com" + row.xpath("./td[2]/a/@href").get("")
        ...
        if next_page:
            url = "https://www.copel.com" + next_page
            yield Request(url, callback=self.payment_history,
                          errback=self.errback_func, dont_filter=True)
        else:
            self.result.update(
